...
![State diagram for the card statuses](mw-gen-payment-card-ib/card_statuses.png)
...
![Obtaining cards initial values diagram](mw-gen-payment-card-ib/create_card_init.png)
...
![Create card diagram](mw-gen-payment-card-ib/create_card_create.png)
...
When the PIN is created on the external card provider's service, we have to tell the cards service that this operation was finished successfully. To do that, we can use the `/cards/{cardId}/!updatePinFlag` endpoint.

See the sequence diagram below:
![Setting the PIN on a physical card diagram](mw-gen-payment-card-ib/create_card_set_pin.png)
...
There are two types of limits supported:
